...
void draw(HDC hdc, RECT* prc){
	...
	drawpc(hdc,prc,&g_hbmPACMAN,&g_hbmPACMAN_MASK,game.pX,game.pY,curdir * PACMANW,drawcyc[game.curCycle %6] * PACMANH );
	DeleteDC(hdcMem);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		...
		case WM_TIMER:
		{
			if (get_input() == 1) has_st = 1;
			int updateRes = update();
			if (updateRes == -1){
				KillTimer(hwnd,IDT_TIMER1);
				MessageBox(hwnd,"You Lost","pacman game",MB_OK);
				DestroyWindow(hwnd);
			}else if (updateRes == 1){
				KillTimer(hwnd,IDT_TIMER1);
				MessageBox(hwnd,"You Won","pacman game",MB_OK);
				DestroyWindow(hwnd);
			}
			RECT rcClient;
			HDC hdc = GetDC(hwnd);
